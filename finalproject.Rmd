...
We can see that most of the wines are of quality rating 6. Very small number of wines are rated 9. We have very small number of top quality rated wines in the dataset.
...
After transfromation and removing outlies it can be observed that quantity of residual sugar roughly follows a bimodal distibution .There is an outlier value in the above plot.Lets explore it further using boxplot:
...
ggplot(aes(x=(residual.sugar)),data=subset(wine,wine$residual.sugar<22))+
  geom_histogram(fill="orange",binwidth = 0.04)+
  xlim(0,quantile(wine$residual.sugar,0.99))+
  scale_x_log10()+
  xlab("Residual Sugar")
...
ggplot(aes(citric.acid),data=wine)+ 
  geom_histogram(fill="orange",binwidth = 0.05)+
...
ggplot(aes(x=volatile.acidity),data=wine)+
  geom_histogram(fill="orange",binwidth = 0.025)+
...
```{r echo=FALSE,warning=FALSE,message=FALSE}
ggplot(aes(x=volatile.acidity),data=subset(wine,wine$volatile.acidity<0.5))+ geom_histogram(fill="orange",binwidth = 0.025)+
...
After removing outliers the distribution of volatile acidity follows the normal distribution with peak around 0.25 .

##6.FREE SULPHUR DIOXIDE

```{r echo=FALSE, warning=FALSE,message=FALSE}
ggplot(aes(x=free.sulfur.dioxide),data=subset(wine,wine$free.sulfur.dioxide<90))+
...
it also follows the normal distribution after removing the outliers.
...
```{r echo=FALSE,warning=FALSE,message=FALSE}
ggplot(aes(x=density),data=subset(wine,wine$density<1.002)) +
  geom_histogram(fill="orange",binwidth = 0.0005)+
  scale_x_continuous(limits = c(0.9871,1.005))

```

Now we have a  roughly normal distribution curve for density. 

##8.pH of wine 

```{r echo=FALSE, warning=FALSE}
ggplot(aes(x=pH),data=subset(wine,wine$pH<3.6 & wine$pH>2.82))+geom_histogram(fill="orange",binwidth = 0.02)
```

Again we can see normal shape of the distribution . Most of the wines have pH around 3.2 .

##9.CHLORIDES

```{r echo=FALSE,warning=FALSE,message=FALSE}
ggplot(aes(x=chlorides),data =subset(wine,wine$chlorides<0.075 & wine$chlorides>0.02))+geom_histogram(fill="orange",binwidth = 0.003)+
  scale_x_continuous(breaks = seq(0.02,0.08,0.01))

```

Again the distribution of chlorides is roughly normal.

##10.SULPHATES

```{r echo=FALSE,warning=FALSE}
ggplot(aes(x=sulphates),data=subset(wine,wine$sulphates<0.78))+
  geom_histogram(fill="orange",binwidth = 0.04)+
  scale_x_continuous(breaks = seq(0.2,0.8,0.04))
```

Most of the wines have sulphate content value around 0.46  .
...
ggplot(aes(x=alcohol, y=quality,color=quality),data=wine)+
  geom_point(alpha=1/14, position = "jitter")
...
```{r echo=FALSE,warning=FALSE}
ggplot(aes(x=volatile.acidity, y=quality,color=quality),data=subset(wine,wine$volatile.acidity<0.7))+
...
```{r echo=FALSE, warning=FALSE}
ggplot(aes(x=density, y=quality,color=quality),
...
```{r echo=FALSE,warning=FALSE,message=FALSE}
ggplot(aes(x=chlorides,y=quality,color=quality),
...
From above plot and summary statistics we can conclude that as the alcohol percentage in wine increases its quality rating also inreases. we can infer that alcohol content is crucial factor to consider while making a good quality wine.
...
Our final plot represents that higher quality wines have high alcohol content, which is already established in final plot 2, and lesser density compratively which is clear from the summary statistics plotted above .
...
In conclusion, using exploratory data analysis, I was able to see new relations, confirm the patterns already stated in the data set description, and found clear some clear trends in dataset.
